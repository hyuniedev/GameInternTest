...
public class BoardController : MonoBehaviour
{
    ...
    public void AutoWin()
    {
        StartCoroutine(m_board.AutoWin(m_gameManager));

    }
    public void AutoLose()
    {
        StartCoroutine(m_board.AutoLose(m_gameManager));
    }
    ...
    public void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (cell.IsOnBox)
                {
                    if (m_gameManager.GetLevelMode() == GameManager.eLevelMode.MOVES)
                        return;
                    else
                        cell.CombackInitCell();
                }
                else
                {
                    m_board.InsertToBox(cell.Item);
                    cell.Free();
                    m_board.CheckEndGame(m_gameManager);
                }
            }
        }
